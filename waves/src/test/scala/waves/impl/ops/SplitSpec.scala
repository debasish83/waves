...
import waves.Operation.Split._
  ...
  "`Split`" should {
    ...
    "while waiting for the first requestMore from a sub-stream" >> {
      ...
      "immediately propagate completion from upstream to downstream and eventually" >> {
        def splitTest(cmd: String) = test(op) { fixture ⇒
        ...
        "complete sub-stream if command for element was `Append`" in splitTest("Append")
        "complete sub-stream if command for element was `Last`" in splitTest("Last")
      }
