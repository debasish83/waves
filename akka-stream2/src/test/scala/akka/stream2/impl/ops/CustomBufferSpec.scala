...
class CustomBufferSpec extends OperationImplSpec {
  ...
  "`CustomBuffer` should" - {
    ...
    "for an op that converts incoming Symbols into Chars and caches the last result for immediate re-dispatch to downstream" - {
      val op = Operation.CustomBuffer[Symbol, Char, Option[Char]]( // consumes from upstream at max rate, produces to downstream at max rate
        seed = None,
    ...
    "for an op that throws from its `compress` function" - {
      val op = Operation.CustomBuffer[Symbol, Char, Option[Char]](
        seed = None,
