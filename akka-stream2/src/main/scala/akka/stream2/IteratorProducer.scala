...
object IteratorProducer {
  def apply[T](iterable: Iterable[T])(implicit ec: ExecutionContext): IteratorProducer[T] = apply(iterable.iterator)
  def apply[T](iterator: Iterator[T])(implicit ec: ExecutionContext): IteratorProducer[T] = new IteratorProducer[T](iterator)
}
...
abstract class AbstractStrictProducer[T](initialBufferSize: Int,
                                         ...
                                         maxRecursionLevel: Int = 32,
                                         maxSyncBatchSize: Int = 128)(implicit executor: ExecutionContext) //FIXME Remove defaults in code
  extends AbstractSyncedProducer[T](initialBufferSize, maxBufferSize) {
...
/**
 * Base class for producers that need a synchronization facility with the subscriber threads.
 */
abstract class AbstractSyncedProducer[T](initialBufferSize: Int, maxBufferSize: Int)
  extends AbstractProducer[T](initialBufferSize, maxBufferSize) {

  protected val locked = new AtomicBoolean // TODO: replace with AtomicFieldUpdater / sun.misc.Unsafe
